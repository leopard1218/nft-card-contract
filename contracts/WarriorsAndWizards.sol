...
contract WarriorsAndWizards is ERC1155, Ownable, ERC1155Supply {
    ...
    /// @dev Change the aggregator address depending on the chain to be deployed.
    ...
    function createBattle(
        string memory _name
    ) external payable returns (Battle memory) {
        require(msg.value >= currentFee(), "Battle fee not covered");
        require(isPlayer(msg.sender), "Please Register Player First"); // Require that the player is registered
     ...
     */
    function _getAvaxPrice() internal view returns (int256) {
     ...
     */
    function currentFee() public view returns (uint256 price) {
        int256 current = _getAvaxPrice();
        price = uint((5 ether * 10 ** 8) / (current * 100));
    }
}
